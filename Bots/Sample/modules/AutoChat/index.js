module.exports = {
    ...
    getReply: function(path, msg, data) { //수신된 채팅에 대한 적당한 답변 반환
    ...
    Chatprocess: function(room, msg, sender, isGroupChat, replier, ImageDB, packageName, threadId, roomList, scriptName) {
        ...
        if (roomList.indexOf(room) != -1) {
            var botRootDir = sdcard + "/" + scriptName;
            var botsavDir = botRootDir + "/[Ai] Leaned DB";
            room = room.trim();
            var botfile = botRootDir + "/[Ai] Leaned DB/" + room + ".txt";
            this.DB.createDir(botsavDir);
            /*
             *(string) room
             *(string) sender
             *(boolean) isGroupChat
            ...
            var data = this.DB.readData(botfile); //저장된 채팅들을 불러옴
            var chat = this.getReply(botfile, msg, data.split("\n"));
            if (chat != null) {
                this.say(chat, replier)
            };
            /*채팅을 학습하는 부분*/
            if (this.isValidData(msg)) { //배울 만한 채팅인 경우,
                var data = this.DB.readData(botfile); //배운 채팅 목록을 가져옴
                if (data == null) { //아직 배운게 없다면
                    this.DB.saveData(botfile, msg); //새로 저장
                } else { //아니면,
                    if (lastSender[room] == sender) { //같은 사람이 연속으로 채팅을 한 경우,
                        this.DB.saveData(botfile, data + " " + msg); //같은 채팅으로 분류
                    } else { //아니면,
                        this.DB.saveData(botfile, data + "\n" + msg); //다른 채팅으로 분류
                    }
                }
            }
